module.exports = async () => {
  ...
  const { getLinksForKeyword } = require('./services/parser')
  const { TWITTER_HANDLE, REDIS_HOSTNAME, REDIS_PORT } = require('./secrets')
  ...
  const getTweetWorker = new RSMQWorker('tweetQueue', options)
  const sendTweetWorker = new RSMQWorker('sendTweets', options)
  const allQueues = await rsmq.listQueues()
  ...
  if (allQueues !== []) {
    ...
    rsmq.createQueue('tweetQueue')
    rsmq.createQueue('sendTweets')
  }
